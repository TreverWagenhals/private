...
int main()
{
    ...
    // it is important to check the return value. for example, when enqueueNDRangeKernel may fail when Work group size
    ...
    clEnqueueReadBuffer(command_queue, bufferC, CL_TRUE, 0, wD*hD*sizeof(float), (void *)D, 0, NULL, NULL);
